import { DoorType } from './model/dungeon-type.model';
...
const createWeightedOptionMap = (obj: Object): WeightedOption[] =>
    Object.entries(obj).map<WeightedOption>(([k, v]) => ({ option: k, weight: v }));

const createUnweightedOptionMap = (obj: Object): WeightedOption[] =>
    Object.entries(obj).map<WeightedOption>(([_, v]) => ({ option: v, weight: 1 }));
...
//#region Starting Area Random Table
...
/**
 * Single source of truth for decoding a `RandomDoorType` code into the
 * concrete (DoorType, isLocked) pair. Replaces the old `extractDoorPropFromCode` switch.
 */
export const DoorTypeFromCode: Record<RandomDoorType, [DoorType, boolean]> = {
    [RandomDoorType.Wooden]:           [DoorType.Wooden, false],
    [RandomDoorType.WoodenLocked]:     [DoorType.Wooden, true],
    [RandomDoorType.Stone]:            [DoorType.Stone, false],
    [RandomDoorType.StoneLocked]:      [DoorType.Stone, true],
    [RandomDoorType.Iron]:             [DoorType.Iron, false],
    [RandomDoorType.IronLocked]:       [DoorType.Iron, true],
    [RandomDoorType.Portcullis]:       [DoorType.Portcullis, false],
    [RandomDoorType.PortcullisLocked]: [DoorType.Portcullis, true],
    [RandomDoorType.Secret]:           [DoorType.Secret, false],
    [RandomDoorType.SecretLocked]:     [DoorType.Secret, true],
};

export const decodeDoorTypeCode = (code: string): [DoorType, boolean] =>
    DoorTypeFromCode[code as RandomDoorType] ?? [DoorType.Other, false];

//#endregion

//#region Beyond Exit Tables

export enum RandomBeyondExit {
    Passage = "P",
    Chamber = "C",
    Stairs = "S",
    Trap = "T",
