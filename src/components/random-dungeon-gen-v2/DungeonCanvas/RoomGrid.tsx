 ...
 *
 * Rect rooms get an axis-aligned rectangular clip; Circle rooms get a
 * clipFunc that traces the room's circumference.
 */
const RoomGrid = ({ room }: RoomGridProps): ReactElement | null => {
  const isCircle = (room as Chamber).shape === RoomShapeType.Circle;
  ...
  if (isCircle) {
    ...
    left = room.transform.position.x * SCALE;
    top = room.transform.position.y * SCALE;
    right = left + room.transform.dimension.x * SCALE;
    bottom = top + room.transform.dimension.y * SCALE;
    clipFunc = (ctx) => {
      ctx.beginPath();
  ...
  // but be defensive in case of arbitrary input.
  ...
  for (let x = startX; x <= endX; x += GRID_CELL) {
    // Skip the room's own outer stroke — drawn by the room shape itself.
